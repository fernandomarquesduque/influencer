...
export async function loginWithCredentials(
  ...
  try {
    ...
    if (!currentUrlAfterGoto.includes('/accounts/login') && !currentUrlAfterGoto.includes('/challenge/')) {
    ...
    const urlChanged = await page.waitForURL(/instagram\.com\/(?!accounts\/login)/, { timeout: 20000 }).then(() => true).catch(() => false);
    log(`URL mudou (saiu do login): ${urlChanged}, atual: ${page.url()}`);
