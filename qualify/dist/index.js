...
function summarizeValidationErrorsForLog(errors) {
    ...
    const maxSnippets = 2;
    const snippets = errors.slice(0, maxSnippets).map((e) => truncateLogLine(e.replace(/^personaSummary /i, 'ps '), 88));
...
const MAIN_CATEGORY_SET = new Set(MAIN_CATEGORY_OPTIONS);
...
export function parseMainCategory(raw) {
    ...
    if (!s)
        return null;
    if (MAIN_CATEGORY_SET.has(s))
        return s;
    try {
        const nfc = s.normalize('NFC');
        if (MAIN_CATEGORY_SET.has(nfc))
            return nfc;
    }
    catch {
        /* ignore */
...
function validateLlmClassification(llm, profile) {
    ...
    if (!parseMainCategory(q.mainCategory)) {
        errors.push(`mainCategory deve ser exatamente uma destas strings (copie literal, sem alterar): ${MAIN_CATEGORY_OPTIONS.join(' | ')}`);
    }
    return errors;
}
...
function buildPrompt(profile, options = {}) {
    ...
    const profileTypeSignals = '"criador" só com pessoa central (nome de influencer, "criador(a) de conteudo digital", etc.). "empresa" para instituicao/marca B2C sem rosto unico: produto empacotado (água mineral, bebida, alimento), nome comercial no titulo, bio de produto/prêmios/site oficial — nao use criador. Também loja, clinica, B2B, agencia em "nos". Bio só com link da marca + discurso institucional indica empresa; URL de pessoa (criador) é outro caso.';
...
async function classifyProfile(profile, model, maxReasoning, promptOverride, onLog, postCtx) {
    ...
    for (let attempt = 1; attempt <= DEFAULT_LLM_REPAIR_ATTEMPTS; attempt++) {
        ...
        const usePostsInPrompt = promptOverride == null &&
            validationErrorsRelateToPersonaSummary(validationErrors) &&
            profileHasPostTextSamples(workingProfile);
