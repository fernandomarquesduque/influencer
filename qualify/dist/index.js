...
function mergeSubsAndPillarsForMainCategoryEvidence(subs, pillars, maxItems = 40) {
    const out = [];
    const seen = new Set();
...
function buildProfileFreeTextForCategoryEvidence(profile) {
    const parts = [];
...
function authorialBlobStrippedForLanguage(profile) {
...
function normalizeFrancInputBlob(blob) {
    return stripInvisibleFromString(blob).replace(/\s+/g, ' ').trim();
}
function francRankedForBlob(blob, francMinLen = 10) {
    const norm = normalizeFrancInputBlob(blob);
    if (norm.length < francMinLen)
        return [];
    return francAll(norm, { minLength: francMinLen });
}
function francScoreForIso3(ranked, iso3) {
...
function languageHintFromHandle(handle) {
    const h = String(handle ?? '').trim().toLowerCase().replace(/^@/, '');
    if (!h)
        return null;
    const latamEs = /(?:^|[._-])(cl|ve|mx|ar|pe|uy|py|bo|ec|gt|cr|pa|do|hn|ni|sv|cu|pr|co)(?:$|[._-])/i.test(h) ||
        /\.(ve|mx|ar|cl|pe|uy|py|bo|ec|gt|cr|pa|do|hn|ni|sv|cu|pr|co)\b/i.test(h);
    if (latamEs)
        return 'es';
    if (/\.(br|com\.br)\b/i.test(h) || /(?:^|[._-])br(?:$|[._-])/i.test(h))
        return 'pt-br';
    if (/\.(uk|com\.uk|com\.hk|com\.sg|com\.au)\b/i.test(h) ||
        /(?:^|[._-])(hk|uk|us|sg|au|nz|ie|tw|jp|kr|cn)(?:$|[._-])/i.test(h) ||
        /(hk|uk|us|sg|au|tw|jp|kr|cn)$/i.test(h)) {
        return 'en';
    }
    if (/\.(it|com\.it)\b/i.test(h))
        return 'it';
    if (/\.(fr|com\.fr)\b/i.test(h))
        return 'fr';
    if (/\.(de|com\.de)\b/i.test(h))
        return 'de';
    return null;
}
...
function detectLanguageHeuristicFromBlob(blob, handle) {
    ...
    if (itStrong && !ptStrong)
        return 'it';
    if (enStrong && !ptStrong)
        return 'en';
    if (esStrong && !ptStrong)
        return 'es';
    if (frStrong && !ptStrong)
        return 'fr';
    if (handleHint && handleHint !== 'pt-br' && !ptStrong)
        return handleHint;
    return null;
}
...
function resolveMappedLanguageFromFrancRanked(ranked, blob, handle) {
    if (ranked.length === 0)
        return null;
    const topCode = ranked[0][0];
    const topScore = ranked[0][1];
    if (!topCode || topCode === 'und')
        return null;
    const low = stripInvisibleFromString(blob).toLowerCase();
    if (topCode === 'por') {
        const spaScore = francScoreForIso3(ranked, 'spa') ?? 0;
    ...
    for (let i = 1; i < ranked.length; i++) {
        ...
        if (!code || code === 'und' || score < FRANC_ALT_MAPPED_MIN_SCORE)
            continue;
        if (code === 'por') {
            const spaScore = francScoreForIso3(ranked, 'spa') ?? 0;
 ...
 */
function resolveLanguageFromFrancProfile(profile) {
    ...
    const fromFranc = ranked.length > 0 ? resolveMappedLanguageFromFrancRanked(ranked, blob, handle) : null;
    if (fromFranc) {
 ...
 */
function maxConfidenceFromAuthorialLetterCount(n) {
    const r = (k, def) => {
 ...
 */
function tryExcludeByAuthorialLanguageMismatch(qualification, profile) {
    ...
    const handle = toHandle(profile.handle ?? '');
    if (authorialTextPredominantlyCjk(blob)) {
        const ptSignals = hasDistinctivePortugueseSignals(low) ||
            hasWeakBrazilianPortugueseSignals(low) ||
    ...
    if (isPortugueseLanguageTag(langRaw)) {
        ...
        const handleHint = languageHintFromHandle(handle);
        const intl = heuristic && normalizeLanguageTagForExclusionRule(heuristic) !== 'pt-br'
            ? heuristic
...
const BEAUTY_CONTENT_PILLAR_FOLDS = new Set([
    'beleza',
    'maquiagem',
    'makeup',
    'make',
...
function isBeautyOnlyContentPillars(pillars) {
    if (pillars.length === 0)
        return false;
    for (const p of pillars) {
        const f = foldMainCategoryKey(String(p ?? '').trim());
        if (!f || !BEAUTY_CONTENT_PILLAR_FOLDS.has(f))
            return false;
    }
    return true;
}
/** Bio/amostras sustentam rotulos de Beleza (regex + sinais ja pontuados em scoreMainCategoriesFromEvidence). */
function profileFreeTextSupportsBelezaEvidence(freeText) {
    const t = foldAsciiWs(freeText);
    if (/\b(beleza|beauty|makeup|cosmeticos|maquiagem|maquiador[ao]?|skincare|cosmetic|cosmetico|perfum|sobrancelh|micropigment|brow|lashes?|manicure|pedicure|cabelo|cabelereiro|cabelereira|visagista|barbeiro|barbeira|dermatolog|estetic|barbearia|barber)\b/iu.test(t)) {
        return true;
    }
    const scores = scoreMainCategoriesFromEvidence([], freeText);
    return categoriesAtMaxEvidenceScore(scores).includes('Beleza');
}
function categoriesAtMaxEvidenceScore(scores) {
    let max = 0;
...
function validateSubCategoriesBeautyAgainstProfileEvidence(qualification, profile) {
...
function validationErrorsSuggestEnrichingWithPostSamples(errors) {
    return errors.some((e) => {
        const x = e.toLowerCase();
        return (x.includes('personasummary') ||
...
function validateMacroLabelArray(field, arr) {
    const errors = [];
    for (const raw of arr) {
        ...
        if (/[,;]/u.test(s)) {
            errors.push(`${field}: sem virgula ou ponto-e-virgula dentro do item — um rotulo por string; item: ${JSON.stringify(s)}`);
        }
        if (s.length > 48) {
...
const GENERIC_CONTENT_PILLAR_FOLDS = new Set([
    'amor',
    'motivacao',
    'dicas',
    'vida',
...
function isContentPillarsOnlyOutrosFallback(contentPillars) {
...
function ensureContentPillarsFallback(qualification, allowOutrosFallback) {
    const pillars = asStringArray(qualification.contentPillars, 100);
    if (pillars.length === 0) {
...
function validateContentPillarsOutrosReservedForLastAttempt(contentPillars, allowOutrosFallback) {
    if (allowOutrosFallback)
        return [];
...
function validationErrorsMentionContentPillars(errors) {
    return errors.some((e) => e.toLowerCase().includes('contentpillars'));
}
function validateContentPillarsNotTooGeneric(contentPillars) {
    if (contentPillars.length === 0)
        return [];
    let gen = 0;
    for (const p of contentPillars) {
        const f = foldMainCategoryKey(String(p ?? '').trim());
        if (f && GENERIC_CONTENT_PILLAR_FOLDS.has(f))
            gen++;
    }
...
function validatePersonaSummaryQuality(personaSummary) {
    const t = foldAsciiWs(personaSummary);
    const errors = [];
    if (/recepcao de (cafe|alimento|comida)/u.test(t)) {
        errors.push('personaSummary: evite "recepcao de cafe/alimentos" — descreva o negocio e o publico com frases naturais');
    }
    if (/recepcoes de/u.test(t)) {
        errors.push('personaSummary: evite "recepcoes de ..." — reescreva o nicho com vocabulario proprio');
    }
    if (/periodo de tempo dedicad/u.test(t)) {
        errors.push('personaSummary: texto tipo template ou truncado ("periodo de tempo dedicado...") — reescreva de forma clara');
    }
...
function profileAuthorialEvidenceLow(profile, includePosts = true) {
    const parts = [];
    ...
    if (bio)
        parts.push(bio);
    for (const field of ['category_name', 'business_category_name', 'overall_category_name']) {
    ...
    if (includePosts) {
        for (const s of buildPostTextSamplesForEvidence(profile)) {
            ...
            if (s.hashtags.length > 0)
                parts.push(s.hashtags.join(' ').toLowerCase());
        }
    }
    return parts.join('\n');
}
function profilePublicTextLow(profile) {
...
function hasLinhaProductBrandHandle(handle) {
    const h = String(handle ?? '').trim().toLowerCase().replace(/^@/, '');
    return /^linha[a-z0-9_]{2,}$/i.test(h);
}
function hasInstagramProductServiceCategory(profile) {
...
function hasInstitutionalThirdPersonBrandVoice(bio, fullName) {
    ...
    if (name.length >= 3) {
        ...
        if (new RegExp(`\\b${escaped}\\s+acredita\\s+que\\b`, 'iu').test(b))
            return true;
    }
    return (/\b\w{3,}\s+acredita\s+que\b/iu.test(b) &&
...
function hasStrongProductBrandSignalsInProfile(profile) {
    const low = profileAuthorialEvidenceLow(profile);
    const handle = String(profile.handle ?? '').toLowerCase();
    const bio = String(profile.biography ?? '').normalize('NFC').toLowerCase();
    const fn = String(profile.full_name ?? '').normalize('NFC');
    if (hasLinhaProductBrandHandle(handle))
        return true;
    if (hasInstagramProductServiceCategory(profile) && hasInstitutionalThirdPersonBrandVoice(bio, fn)) {
        return true;
    }
    const cleaningContext = /\b(limpeza|detergente|sab[aã]o|lavanderia|lou[cç]a|amaciante|desinfetante|multiuso|alvejante|limpador|higiene\s+dom[eé]stica|produtos?\s+de\s+limpeza|linha\s+roupas?|linha\s+lou[cç]a|linha\s+casa|rende\s+mais|barraca\s+do\s+detergente|minuano)\b/iu.test(low) || hasLinhaProductBrandHandle(handle);
    const cleaningVoice = hasInstitutionalThirdPersonBrandVoice(bio, fn) ||
        /\b(acredita\s+que|rende\s+mais|nossa\s+linha|nossos?\s+produtos?|linha\s+de\s+produtos?|e-?commerce|loja\s+oficial|compre\s+agora)\b/iu.test(low);
    if (cleaningContext && cleaningVoice)
        return true;
    const skinContext = /\b(cosm[eé]tic|skincare|maquiagem|perfum|pele|tallow|sebo\s+bovino)\b/iu.test(low);
    const skinVoice = /\bmarca\s+de\s+(cosm[eé]tic|beleza|skincare|produtos?)\b/iu.test(low) ||
        /\b(nossa\s+marca|somos\s+uma\s+marca)\b/iu.test(low);
    if (skinContext && skinVoice)
        return true;
    return false;
}
/** Sinais fortes de negocio/instituicao — empresa/marca/midia plausivel. */
function hasStrongEmpresaSignalsInProfile(profile) {
    const low = profilePublicTextLow(profile);
    const handle = String(profile.handle ?? '').toLowerCase();
    if (/\b(restaurante|bar\b|doceria|pizzaria|lanchonete|café|cafe\b|padaria|delivery|peça já|peca ja|pedidos pelo|whatsapp.*pedido|loja oficial|varejo|clinica|clínica|hospital|agencia de|agência de|escritório cont|b2b|ministério|ministerio|prefeitura|corpo de bombeiros|polícia militar|exército|exercito|oficial\b.*\b(pm|eb|cbm)|bagels?|sandwiches|spreads|coffee shop|square\.site|jornal\b|giornale|informazione|edicola|notizie|liga\s+nacional|liga\s+de\s+|empresa\s+de\s+entretenimento|entretenimento\b.*\b(eventos|licenciamento|media)|licenciamento\b|lnr\s+(eventos|media|music|licenciamento)|federa[cç][aã]o|associa[cç][aã]o|instituto|fund[aá][cç][aã]o|canal oficial|conta oficial|detergente|sab[aã]o|produtos?\s+de\s+limpeza|linha\s+roupas?|linha\s+lou[cç]a|linha\s+casa|rende\s+mais|e-?commerce)\b/iu.test(low)) {
        return true;
    }
    if (/\bempresa\b/iu.test(low))
        return true;
    if (hasLinhaProductBrandHandle(handle) || hasStrongProductBrandSignalsInProfile(profile))
        return true;
    if (hasInstagramProductServiceCategory(profile))
        return true;
    if (/_restaurante|_loja|_store|_shop\b/i.test(handle))
        return true;
    return false;
}
/** Pessoa fisica / criador de conteudo — musica, esporte, arte, influencia. */
function hasStrongCreatorPersonSignalsInProfile(profile) {
    const low = profileAuthorialEvidenceLow(profile);
    if (hasStrongEmpresaSignalsInProfile(profile) || hasStrongProductBrandSignalsInProfile(profile))
        return false;
    if (/\b(criador(a)? de conte[uú]do|influencer|youtuber|blogueir[ao]|digital influencer|content creator)\b/iu.test(low)) {
        return true;
    }
    ...
    const fn = String(profile.full_name ?? '').trim();
    if (fn.length >= 4 && /\s/.test(fn) && /\b(sou|sou o|sou a|me chamo)\b/iu.test(low))
        return true;
    return false;
}
/**
 ...
 */
function correctProfileTypeWhenEmpresaOrInstitutionalSignals(profile, qualification) {
    if (hasStrongCreatorPersonSignalsInProfile(profile))
        return;
    const low = profilePublicTextLow(profile);
...
function qualificationProfileTypeTriggersAutoExclusion(qualification) {
    const pt = String(qualification.profileType ?? '').trim().toLowerCase();
...
function correctProfileTypeWhenProductBrand(profile, qualification) {
    ...
    const productOrSkinContext = /\b(cosm[eé]tic|skincare|maquiagem|perfum|pele|skin\s*care|body\s*care|hair\s*care|tallow|sebo\s+bovino|sebo bovino|[oó]leos?\s+nat|ingredient|f[óo]rmula|produtos?\s+nat|vegano|clean\s*beauty|dermato)\b/iu.test(low);
    const institutionalBrandVoice = /\bmarca\s+de\s+(cosm[eé]tic|beleza|skincare|produtos?)\b/iu.test(low) ||
        /\b(nossa\s+marca|somos\s+uma\s+marca|a\s+marca\s+\S+\s+(cria|oferece|desenvolve|acredita))\b/iu.test(low) ||
    ...
    const d2cCosmeticsBrand = productOrSkinContext &&
    ...
    const cleaningOrHomeProductContext = /\b(limpeza|detergente|sab[aã]o|lavanderia|lou[cç]a|amaciante|desinfetante|multiuso|alvejante|limpador|higiene\s+dom[eé]stica|produtos?\s+de\s+limpeza|linha\s+roupas?|linha\s+lou[cç]a|linha\s+casa|rende\s+mais|barraca\s+do\s+detergente)\b/iu.test(low) ||
        /\blinha[a-z0-9_]{2,}\b/iu.test(handle);
    const consumerCleaningBrandVoice = /\b(acredita\s+que|rende\s+mais|nossa\s+linha|nossos?\s+produtos?|linha\s+de\s+produtos?|e-?commerce|loja\s+oficial|compre\s+agora)\b/iu.test(low) ||
        (/\b\w{3,}\s+acredita\s+que\b/iu.test(low) &&
            /\b(tempo\s+vale|render|deseja\s+fazer|produto|m[aá]ximo)\b/iu.test(low));
    const d2cCleaningBrand = cleaningOrHomeProductContext &&
...
function correctProfileTypeWhenPersonaDescribesProductBrand(qualification) {
    const pt = String(qualification.profileType ?? '').trim().toLowerCase();
    if (pt !== 'criador' && pt !== 'pessoal')
        return;
    const ps = foldAsciiWs(String(qualification.personaSummary ?? ''));
    if (/\bmarca\s+de\s+cosm(e|é)tic/.test(ps) ||
...
function normalizeProfileTypeLeakage(qualification) {
    const raw = String(qualification.profileType ?? '').trim().toLowerCase();
    if (!raw || !raw.includes('|')) {
        if (PROFILE_TYPE_CANONICAL.has(raw))
            qualification.profileType = raw;
        return;
    }
    const parts = raw
        .split('|')
    ...
    if (parts.length === 0)
        return;
    if (parts.includes('criador'))
        qualification.profileType = 'criador';
    else if (parts.includes('pessoal'))
        qualification.profileType = 'pessoal';
    else
        qualification.profileType = parts[0];
}
function normalizeClassification(raw, profile, model, _maxReasoning, languageFromFranc, options = {}) {
    ...
    let status = statusRaw === 'insufficient_data' ? 'insufficient_data' : 'done';
    const letters = countAuthorialLettersForProfileEvidence(profile);
...
function validateLlmClassification(llm, profile, options = {}) {
    const allowOutrosFallback = options.allowContentPillarOutrosFallback === true;
    const errors = [];
...
async function fetchJson(url, init, timeoutMs, retries = 2) {
    let lastError = null;
...
function isQualifyQueuePrefetchEnabled() {
...
function includeContentForPendingFetch(batchLimit) {
    if (batchLimit > 1)
        return false;
    return isQualifyQueueIncludeContentEnabled();
}
function getPendingFetchTimeoutMs(batchLimit) {
...
async function withResilientApiFetch(emitLog, shouldStop, context, fn) {
    let attempt = 0;
    while (true) {
        if (shouldStop?.())
        ...
        try {
            ...
            emitLog(`(${context}) erro na API: ${truncateLogLine(message, 160)} — tentativa ${attempt}, nova tentativa em ${Math.round(waitMs / 1000)}s`);
...
async function fetchPendingFromApi(apiBase, ingestKey, limit, offset = 0, refreshAll = true, mainCategory, mediaLimitOverride, includeContent = false) {
    ...
    const mediaLimit = mediaLimitOverride != null && Number.isFinite(mediaLimitOverride)
    ...
    }, getPendingFetchTimeoutMs(limit), getApiFetchRetries());
...
async function fetchLlmMainCategoryLabelsFromApi(apiBase, ingestKey) {
    const data = await fetchJson(`${apiBase}/crawl/collector-llm-main-categories`, {
        method: 'GET',
        headers: {
            'X-Collector-Key': ingestKey,
        },
...
let mainCategoriesRefreshPromise = null;
function getMainCategoryTaxonomyFallback() {
...
async function refreshMainCategoriesCache(apiBase, ingestKey) {
    if (mainCategoriesRefreshPromise)
...
async function getLlmMainCategoryLabelsCached(apiBase, ingestKey, forceRefresh = false) {
    const now = Date.now();
...
let coverageStatsCache = null;
let coverageStatsRefreshPromise = null;
async function fetchLlmCoverageStatsCached(apiBase, ingestKey, forceRefresh = false) {
    const now = Date.now();
    const cache = coverageStatsCache;
    if (!forceRefresh && cache != null && now - cache.fetchedAt < COVERAGE_STATS_CACHE_TTL_MS) {
        return cache.data;
    }
    if (coverageStatsRefreshPromise)
...
function displayFieldsFromProfileForResultRow(profile) {
    ...
    const qualOk = llmObj?.qualification != null &&
...
function shouldExcludeByLanguageAndCreatorRules(qualification) {
    ...
    if (profileT !== 'pessoal' && profileT !== 'criador')
        return true;
    return false;
}
function buildAutoExclusionReason(qualification) {
...
function qualificationMatchesAutoExclusionPolicy(qualification) {
    return shouldExcludeByLanguageAndCreatorRules(qualification);
}
function resolveAutoExclusionReason(qualification, profile) {
...
function tryExtractProfileTypeFromLooseAssistantJson(text) {
    const compact = stripInvisibleFromString(text).replace(/\s+/g, ' ');
    const m = /["']?profileType["']?\s*:\s*["']?(criador|pessoal|empresa|noticia|marca|midia)["']?/iu.exec(compact);
    return m ? m[1].trim().toLowerCase() : null;
}
/** Corpo minimo para seguir o fluxo da fila (regra de exclusao por profileType) sem nova chamada ao modelo. */
function buildEmpresaStubParsedWhenJsonInvalid() {
    return {
        confidence: 0.35,
        status: 'insufficient_data',
        qualification: {
...
function qualificationProfileTypeIsEmpresa(llm) {
    const q = asObject(llm.qualification);
...
async function classifyProfile(profile, model, maxReasoning, promptOverride, onLog, postCtx) {
    ...
    const runInitialGeneration = async () => {
        ...
        if (!parsed) {
            ...
            if (loosePt === 'empresa') {
                logStep('>> local: JSON invalido mas profileType empresa detectado no texto — sem 2a chamada Ollama (regra de exclusao na fila)');
        ...
        if (!parsed) {
            ...
            const retry = await ollamaClient.chat({
                ...
                messages: [
                    ...
                    {
                        role: 'user',
                        content: 'Reescreva AGORA apenas no formato JSON solicitado. Sem chave "language". Demais campos: valores plausiveis.',
                    },
                ],
                stream: false,
                options: getOllamaChatOptions(),
    ...
    const pillarValidationOpts = { allowContentPillarOutrosFallback: false };
    const pillarLastAttemptOpts = { allowContentPillarOutrosFallback: true };
    let parsed = await runInitialGeneration();
    logStep('>> local: normalizar + validar regras...');
    let llm = normalizeClassification(parsed, workingProfile, model, maxReasoning, languageFromFranc, pillarValidationOpts);
    ...
    if (qualificationProfileTypeTriggersAutoExclusion(qualAfterNorm) &&
        hasStrongEmpresaSignalsInProfile(workingProfile)) {
        logStep(`>> local: ${String(qualAfterNorm.profileType ?? 'empresa')} confirmado na bio — sem reparo LLM; fila aplica exclusao automatica`);
        return llm;
    }
    if (qualificationProfileTypeIsEmpresa(llm)) {
        logStep('>> local: empresa sem sinais fortes na bio — tentando reparo LLM (possivel falso positivo do modelo)');
    }
    ...
    for (let attempt = 1; attempt <= DEFAULT_LLM_REPAIR_ATTEMPTS; attempt++) {
        ...
        logStep(`>> Ollama ${ollamaTag}: reparo validacao #${attempt}/${DEFAULT_LLM_REPAIR_ATTEMPTS}...`);
        ...
        llm = normalizeClassification(repairedParsed, workingProfile, model, maxReasoning, languageFromFranc, pillarValidationOpts);
...
async function runOneAtATimeBatch(options, ctx) {
...
async function runBatch(options = {}) {
    ...
    const useOneAtATimeQueue = concurrency === 1 &&
        !onlyWithLlm &&
        !useCategoryFilter &&
    ...
    if (needsCoverageForOffset) {
        ...
        const covRes = await withResilientApiFetch(emitLog, options.shouldStop, 'stats', () => fetchLlmCoverageStatsCached(apiBase, ingestKey));
        if (!covRes.ok) {
            return {
                total: progress.total,
                done: progress.done,
    ...
    const firstBatchRes = await withResilientApiFetch(emitLog, options.shouldStop, 'fila', () => fetchPendingFromApi(apiBase, ingestKey, fetchBatchSize, listOffset, refreshAll, useCategoryFilter ? mainCategoryFilter : undefined, undefined, queueIncludeContent));
    if (!firstBatchRes.ok) {
        return {
            total: progress.total,
            done: progress.done,
    ...
    if (firstBatch.items.length === 0 && queueOffset > 0) {
        ...
        listOffset = 0;
        const retryBatchRes = await withResilientApiFetch(emitLog, options.shouldStop, 'fila', () => fetchPendingFromApi(apiBase, ingestKey, fetchBatchSize, listOffset, refreshAll, useCategoryFilter ? mainCategoryFilter : undefined, undefined, queueIncludeContent));
        if (!retryBatchRes.ok) {
            return {
                total: progress.total,
                done: progress.done,
    ...
    const flushQueueSkipBurst = () => {
        if (queueSkipBurstPages === 0)
            return;
        emitLog(`(fila) ${queueSkipBurstPages} pagina(s), ${queueSkipBurstProfiles} perfil(is) ja visto(s) nesta execucao — offset agora ${listOffset}.`);
        queueSkipBurstPages = 0;
        queueSkipBurstProfiles = 0;
    };
    const fetchQueuePage = async (offset, reason) => {
        emitLog(`(fila) ${reason} — consultando API offset ${offset} (limite ${fetchBatchSize})…`);
        const t0 = Date.now();
        const pageRes = await withResilientApiFetch(emitLog, options.shouldStop, 'fila', () => fetchPendingFromApi(apiBase, ingestKey, fetchBatchSize, offset, refreshAll, useCategoryFilter ? mainCategoryFilter : undefined, undefined, queueIncludeContent));
        if (!pageRes.ok) {
            return null;
        }
        const batch = pageRes.value;
        const sec = ((Date.now() - t0) / 1000).toFixed(1);
        emitLog(`(fila) offset ${offset}: ${batch.items.length} perfil(is) nesta pagina (~${batch.totalPending} pendentes no total) em ${sec}s`);
        return batch;
    };
    while (true) {
        if (options.shouldStop?.()) {
        ...
        if (todo.length === 0) {
            if (currentBatch.length === 0) {
                if (listOffset > queueOffset) {
                    ...
                    if (queueWrapsWithoutWork >= 2) {
                        emitLog('(fila) encerrando: varreu o fim da fila duas vezes sem processar nenhum perfil novo (fila vazia ou estagnada).');
                        break;
                    }
            ...
            if (shard.active && shardHandlesOnPage.length === 0 && isQualifyVerboseLog()) {
                emitLog(`(fila) pagina sem perfis desta particao ${formatShardLabel(shard)} — offset ${listOffset}.`);
            }
...
function formatOllamaConfigMessage() {
...
async function startUiServer() {
    ...
    const refreshCoverageStats = async (forceRefresh = false) => {
        ...
        try {
            const stats = await fetchLlmCoverageStatsCached(state.apiBaseUsed, getIngestKeyOrThrow(), forceRefresh);
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (method === 'GET' && url.pathname === '/api/llm-main-categories') {
            try {
                ...
                const data = await getLlmMainCategoryLabelsCached(getApiBaseOrThrow(), getIngestKeyOrThrow(), forceRefresh);
                res.writeHead(200, {
    ...
    logOllamaConfig();
    pushLog(`Painel local :${port} — dados de perfil vêm de ${state.apiBaseUsed} (nao confundir com a API Node em :3500).`);
    void refreshCoverageStats();
