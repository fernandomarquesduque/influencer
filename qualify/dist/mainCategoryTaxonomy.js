 ...
 */
export function expandSubcategoryTokensForLookup(raw) {
    ...
    for (const p of [whole, ...parts]) {
        if (!p || p.length < MIN_SUBCATEGORY_TOKEN_LEN)
            continue;
        if (seen.has(p))
            continue;
        seen.add(p);
...
const AMBIGUOUS_SUBCATEGORY_SIGNAL_FOLDS = new Set([
    'natureza',
    'bar',
    'arte',
    'artes',
...
function weightForSubcategoryTokenMatch(foldToken) {
...
export function isSubcategoryMappedToMain(raw) {
    for (const tok of expandSubcategoryTokensForLookup(String(raw ?? '').trim())) {
        if (SUBCATEGORY_FOLD_TO_MAIN.has(tok))
            return true;
    }
    return false;
}
 ...
 */
export function formatSubcategoryTaxonomyHintsForPrompt() {
...
export function scoreSubCategoriesToMainCategories(subCategories) {
    ...
    for (const raw of subCategories) {
        for (const token of expandSubcategoryTokensForLookup(String(raw ?? '').trim())) {
            ...
            if (mapped)
                bumpEvidenceScore(scores, mapped, weightForSubcategoryTokenMatch(token));
        }
    }
    return scores;
}
...
export function scoreMainCategoriesFromEvidence(subCategories, freeText) {
    ...
    for (const raw of subCategories) {
        for (const tok of expandSubcategoryTokensForLookup(String(raw ?? '').trim())) {
            ...
            if (cat)
                bumpEvidenceScore(scores, cat, weightForSubcategoryTokenMatch(tok));
        }
    }
...
export function resolveMainCategoryWithEvidence(snappedMainCanonical, subCategories, freeTextProfileContext) {
    ...
    if (ls > 0)
        scores.set('Lifestyle', Math.max(0, ls - 1));
    const maxScore = maxEvidenceScore(scores);
...
export function snapMainCategoryToTaxonomy(raw) {
    ...
    if (compoundParts.length > 1) {
        for (const part of compoundParts) {
            ...
            if (partSnap && partSnap !== MAIN_CATEGORY_FALLBACK)
                return partSnap;
        }
    }
    return MAIN_CATEGORY_FALLBACK;
